...
url = (
    ...
    "&timezone=America%2FNew_York&past_days=2&forecast_days=2"
)
...
times = [
    datetime.datetime.fromisoformat(t).replace(tzinfo=ZoneInfo("America/New_York"))
    for t in h["time"]
]

aft = [
    i for i, t in enumerate(times)
    if t.date() == now.date() and 12 <= t.hour <= 17
]
...
cloud = avg("cloud_cover", aft)
rain = max(h["precipitation_probability"][i] for i in aft)
wind = avg("wind_speed_10m", aft)
direction = avg("wind_direction_10m", aft)

recent = [
    i for i, t in enumerate(times)
    if 0 <= (now - t).total_seconds() / 3600 <= 6
]

prior = [
    i for i, t in enumerate(times)
    if 18 <= (now - t).total_seconds() / 3600 <= 30
]

dew_drop = avg("dew_point_2m", prior) - avg("dew_point_2m", recent)
temp_drop = avg("temperature_2m", prior) - avg("temperature_2m", recent)

post = max(0, min(25, dew_drop * 1.6 + temp_drop * 0.5))
...
if score >= 75:
    body = (
        f"Score {score}/100 • {temp:.0f}°F • "
        f"{cloud:.0f}% clouds • {rain:.0f}% rain risk. "
    ...
    req = urllib.request.Request(
        ...
        method="POST",
        headers={
            "Title": "🦋 High monarch potential in Hickory",
            "Priority": "high"
        }
    )
